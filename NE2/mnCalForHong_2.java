...
public class mnCalForHong_2 {
    ...
    public mnCalForHong_2(double alpha, int d, int k, ArrayList<Double> gammas, ArrayList<Double> izsp){
        this.alpha = alpha;
        this.k = k;
        this.d = d;
        this.gammas = gammas;
...
//        System.out.println("left: "+left+" right: "+right);
        ...
        double aa = (k-1)*inverseCumulativeProbabilityforSTD(-right/asbar) - alpha;
        for (int j = 0; j < d; j++){
            aa = aa + inverseCumulativeProbabilityforSTD(-right/as[j]);
        }
        if (left > right || aa > 0){
