...
//        };
                        ...
                        for (int e = 1; e <= bound[4][1]; e++){
                            for (int f = 1; f <= bound[5][1]; f++){
                                if (a*ac+b*bc+c*cc==budget1 && d*dc+e*ec+f*fc==budget2){
                                    ...
                                    alt[5] = f;
                                    listALT.add(alt);
                                    System.out.println("第"+altcount+"个方案为： "+ Arrays.toString(alt)+" "+(a*ac+b*bc+c*cc)+"=?"+budget1+" "+(d*dc+e*ec+f*fc)+"=?"+budget2);
                                    altcount+=1;
                                }
                            }
                        }
        ...
        System.out.println("总方案数为： "+altcount+" "+listALT.size());
...
//        CCSP1 y1 = new CCSP1();
        ...
        int rows = repeat;
        int cols = 4; // 0index 1输出的方案 2样本量 3seed
        double[][] data = new double[rows][cols];

        for (int count = 0; count < repeat; count++){
            System.out.println("di"+count+"ci");
            long tempSeed = R.nextLong();
            NE4_P2 y1 = new NE4_P2(alpha, d, gammas, listALT, tempSeed);
            y1.run();
...
//            }
        ...
        System.out.println("正确率： "+correct*1d/repeat+" 样本量0.95的置信区间为:  " + sampleSize/repeat + "±" + halfinterval);


        String filePath = "C:/Users/BRZ/Desktop/NE/NE4_0517/NE5_0525/NE4P2"+seed+".csv";  // 结果输出路径
        try (FileWriter writer = new FileWriter(filePath)) {
            StringBuilder header = new StringBuilder();
//            for (int j = 1; j <= cols; j++) {
//                header.append("Column").append(j);
//                if (j < cols) {
//                    header.append(",");
//                }
//            }
            ...
            System.out.println("CSV 文件已成功生成：" + filePath);
        } catch (IOException e) {
            e.printStackTrace();
        }
