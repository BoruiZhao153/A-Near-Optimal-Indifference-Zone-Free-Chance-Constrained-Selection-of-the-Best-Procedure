...
//    }
    ...
    public void run() {
        ...
        ArrayList<Double> holdingCostCount = new ArrayList<>();


        // 医生数量配置
//        int numServerConRoom = 3; //4.5 18
//        int numServerConCubicle = 3; // 6 24
//        int numServerResuscitation = 3; // 8 24
//        int numServerLabtest = 3; //3.5 10.5
//        int numServerRegistration = 2; // 1 2
//        int numServerTriage = 2;  // 1 2
        ...
        double lambda = (double) 1 ; // 泊松分布的平均间隔时间（3分钟）

        double currentTime = 0; // 仿真开始时间（分钟）
        ArrayList<Integer> patientInED = new ArrayList<>();////zhong
        //设置刚进入ED的患者的状态
...
//            System.out.println(i+" "+arrivalTime);
            ...
            arrivalTimeCurrentNode.add(null); // 当前节点到达时间
            serviceStartTimeCurrentNode.add(null); // 当前节点服务开始时间
            serviceDurationCurrentNode.add(null); // 当前节点服务时长
            totalWaitTimeAfterLeavingCurrentNode.add(0.0); // 离开当前节点前的总等待时间
            leaveEDTime.add(null); // 离开急诊科时间
            patientStatus.add("notEnteredED");
            patientConsultationDoctor.add(-1); // 初始化为-1，表示尚未分配医生
            firstTreatmentTime.add(null);
        }
        int count = 0;      //zhong
        long ____t1 = System.nanoTime();

//        while (patientStatus.contains("notEnteredED") || patientStatus.contains("inED")) {
...
//            for (int i = 0; i < numberOfPatients; i++) {
//                if (!patientStatus.get(i).equals("leftED") && nextNodeTime.get(i) < earliestNextNodeTime) {
//                    earliestNextNodeTime = nextNodeTime.get(i);
//                    nextPatientIndex = i;
//                }
//            }
...
//            }
            ...
            if(count == 0 ){            //zhong
                patientInED.add(0);     //zhong
                patientInED.add(1);     //zhong
                earliestNextNodeTime = nextNodeTime.get(0);     //zhong
                nextPatientIndex = 0;       //zhong
                count = 1;      //zhong
            }else{      //zhong
                for(int i = 0; i < patientInED.size();i++){     //zhong
                    if(nextNodeTime.get(patientInED.get(i)) < earliestNextNodeTime) {       //zhong
                        earliestNextNodeTime = nextNodeTime.get(patientInED.get(i));        //zhong
                        nextPatientIndex = patientInED.get(i);      //zhong
                    }       //zhong
                }       //zhong
                if(nextPatientIndex == count && count < numberOfPatients-1){      //zhong
                    patientInED.add(count+1);       //zhong
                    count++;        //zhong
                }       //zhong
            }       //zhong
...
//            System.out.println("ID: " + patientId.get(nextPatientIndex)+"  T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                    +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                    +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                    +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                    +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
//                    +"  Doct: " + (patientConsultationDoctor.get(nextPatientIndex) == -1 ? "None" : patientConsultationDoctor.get(nextPatientIndex)));
            ...
            if (patientStatus.get(nextPatientIndex).equals("notEnteredED")&& !Objects.equals(patientType.get(nextPatientIndex), "C1")) {
                ...
                // 3.设置到达当前节点的时间
                // 4.更改患者状态（离开系统前不会变）
                // 5.设置在当前节点的等待时间（前往下一节点前不会变化）
                // 6.更新患者在当前节点开始服务时间与前往下一节点时间
...
//                    int insertIndex = registrationQueue.size();
//                    System.out.println(numServerRegistration);
...
//                sortingTime = sortingTime + System.nanoTime() - ____t;
                    ...
                    registrationQueue.add(nextPatientIndex);
//                    System.out.println("insertIndex "+(registrationQueue.size()));
                }

//                System.out.println(nextPatientIndex+"被插入registration, 插入后队列长度为："+registrationQueue.size());
//                for (int i = 0; i < registrationQueue.size(); i++){
//                    System.out.println("                "+i+" "+registrationQueue.get(i)+" "+patientType.get(registrationQueue.get(i))
//                            +" "+serviceStartTimeCurrentNode.get(registrationQueue.get(i))+" "+nextNodeTime.get(registrationQueue.get(i)));
//                }
//                System.out.println("1-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
...
//                System.out.println("2-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
//                        +"  Doct: " + (patientConsultationDoctor.get(nextPatientIndex) == -1 ? "None" : patientConsultationDoctor.get(nextPatientIndex)));
                ...
                // 1.累积上一节点的等待时间、同时从会诊室医生队列踢出
                // 2.设置患者当前节点与下一节点
                // 3.设置到达当前节点的时间、离开ED的时间（当前时间）、离开该节点的时间（无穷大）
                // 4.更改患者状态（离开系统前不会变）
                // 5.设置在当前节点的服务开始时间（当前时间），等待时间（无穷大）
                // 6.更新患者在当前节点开始服务时间与前往下一节点时间
...
//                    int insertIndex = triageQueue.size();
...
//                System.out.println(nextPatientIndex+"被插入triage, 插入后队列长度为："+(triageQueue.size()));
//                for (int i = 0; i < triageQueue.size(); i++){
//                    System.out.println("                "+i+" "+triageQueue.get(i)+" "+patientType.get(triageQueue.get(i))
//                            +" "+serviceStartTimeCurrentNode.get(triageQueue.get(i))+" "+nextNodeTime.get(triageQueue.get(i)));
//                }
//                System.out.println("3-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
...
//                System.out.println("4-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
...
//                System.out.println("5-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
//                        +"  Doct: " + (patientConsultationDoctor.get(nextPatientIndex) == -1 ? "None" : patientConsultationDoctor.get(nextPatientIndex)));
                ...
                // 1.累积上一节点的等待时间、同时从上一节点队列踢出
                // 2.更新当前节点和下一节点
                // 3.设置到达当前节点的时间
                // 4.设置在当前节点的等待时间（前往下一节点前不会变化）
                // 5.更新患者在当前节点开始服务时间与前往下一节点时间
                if (Objects.equals(currentNode.get(nextPatientIndex), "resuscitation")){
                    if (firstTreatmentTime.get(nextPatientIndex)!=null){
                        System.out.println("计算去过急诊室的c1c2患者的ft时出现问题！！！");
                    }
                    firstTreatmentTime.set(nextPatientIndex,serviceStartTimeCurrentNode.get(nextPatientIndex)-enterEDTime.get(nextPatientIndex));
//                    System.out.println("4-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+" 设置了FT"+firstTreatmentTime.get(nextPatientIndex));
                }
//                if (!Objects.equals(currentNode.get(nextPatientIndex), "labtest")){
//                    firstTreatmentTime.set(nextPatientIndex,serviceStartTimeCurrentNode.get(nextPatientIndex)-enterEDTime.get(nextPatientIndex));
//                }
...
//                System.out.println("6-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
//                        +"  Doct: " + (patientConsultationDoctor.get(nextPatientIndex)));
...
//                System.out.println("7-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
//                        +"  Doct: " + (patientConsultationDoctor.get(nextPatientIndex) == -1 ? "None" : patientConsultationDoctor.get(nextPatientIndex)));
                ...
                if (currentNode.get(nextPatientIndex)== "resuscitation"){
                    ...
                    firstTreatmentTime.set(nextPatientIndex,serviceStartTimeCurrentNode.get(nextPatientIndex)-enterEDTime.get(nextPatientIndex));
//                    System.out.println("6-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+" 设置了FT"+firstTreatmentTime.get(nextPatientIndex));
                }
                totalWaitTimeAfterLeavingCurrentNode.set(nextPatientIndex, totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)
                        + serviceStartTimeCurrentNode.get(nextPatientIndex) - arrivalTimeCurrentNode.get(nextPatientIndex));
                deleteFromQueue(nextPatientIndex, currentNode.get(nextPatientIndex), nodeQueueMap);
                ...
                addToQueue(nextPatientIndex, currentNode.get(nextPatientIndex), nodeQueueMap, patientType, serverCount, nextNodeTime,
                        currentTime, serviceStartTimeCurrentNode, serviceDurationCurrentNode);
//                System.out.println("8-ID: " + patientId.get(nextPatientIndex)+" T: " + patientType.get(nextPatientIndex)+"  CN: " + currentNode.get(nextPatientIndex)
//                        +"  NN: " + nextNode.get(nextPatientIndex)+"  NNT: " + nextNodeTime.get(nextPatientIndex)+"  ATCN: " + arrivalTimeCurrentNode.get(nextPatientIndex)
//                        +"  SSTCN: " + serviceStartTimeCurrentNode.get(nextPatientIndex)+"  SD: " + serviceDurationCurrentNode.get(nextPatientIndex)
//                        +"  TWT: " + totalWaitTimeAfterLeavingCurrentNode.get(nextPatientIndex)+"  EnterT: " + enterEDTime.get(nextPatientIndex)
//                        +"  LeaveT: " + leaveEDTime.get(nextPatientIndex) + "  Status: " + patientStatus.get(nextPatientIndex)
...
//            System.out.println(i+" "+patientType.get(i));
...
//        System.out.println("sT: "+sortingTime/1000000);
//        System.out.println("tT: "+(endTime-startTime));
    }
    // 确定下一个节点的方法
        ...
        switch (currentNode) {
            ...
            case "triage":
                if (patientType.equals("C5") || patientType.equals("C4")) {
                    ...
                    if (R.nextDouble() < 0.05) { // 50%的概率C2恶化 ????????????????????
                        return "resuscitation";
                    }else{
                        return "conCubicle";
                    }
            ...
            case "conRoom":
                if(patientType.equals("C4")){
                    if (R.nextDouble() < 0.15) { // 50%的概率需要进一步治疗
                        return "labtest";
                    } else {
                        return "discharge";
                    }
                }else if(patientType.equals("C5")){
                    if (R.nextDouble() < 0.03) { // 50%的概率需要进一步治疗
                        return "labtest";
                    } else {
                        return "discharge";
                    }
            ...
            case "conCubicle":
                if(patientType.equals("C2")){
                    if (R.nextDouble() < 0.1) { // 50%的概率需要进一步治疗
                        return "labtest";
                    } else {
                        return "discharge";
                    }
                }else if(patientType.equals("C3")){
                    if (R.nextDouble() < 0.2) { // 50%的概率需要进一步治疗
                        return "labtest";
                    } else {
                        return "discharge";
                    }
    ...
    public static double getProcessingTime(String patientType, String currentNode) {
        switch (currentNode) {
            ...
            case "triage":
                if (patientType.equals("C2")) {
                    return 3.5;
//                    return triangularDistribution(2.5, 3, 3.5);
...
//                    return triangularDistribution(9, 9.25, 9.5);
                }else {
...
//            System.out.println(patientId+"被插入"+currentNode+"的"+insertIndex+" 插入后队列长度为："+queue.size());
...
//            System.out.println(patientId+"被插入"+currentNode+"的第"+doctorIndex+"个server的"+insertIndex+" 插入后队列长度为："+queue.size());
//            for (int i = 0; i < queue.size(); i++){
//                System.out.println("                "+i+" "+queue.get(i)+" "+patientType.get(queue.get(i))+" "+nextNodeTime.get(queue.get(i)));
//            }
        }
