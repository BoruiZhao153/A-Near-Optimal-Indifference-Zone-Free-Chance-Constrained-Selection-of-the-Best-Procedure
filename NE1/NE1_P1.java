...
//        R.setSeed(234234);
    ...
    public NE1_P1(double alpha, double gamma, double feasibleThreshold, ArrayList<Double> ppmu, ArrayList<Double> ppsigma2,
...
//                    double sampleSPp= R.nextGaussian() * Math.sqrt(spsigma2.get(I.get(i)))+spmu.get(I.get(i));
//                    if(sampleSPp>feasibleThreshold){
//                        sampleSP[i] = 1;
//                    }else{
//                        sampleSP[i] = 0;
//                    }
//                    sampleSP[i]= bernoullisample(spp.get(I.get(i)));
                }
...
//            double epsilon = Math.sqrt(-Math.log(alpha/(k*(Math.log(t)/Math.log(2))*(Math.log(t)/Math.log(2))))/(2*t));
            double gtildet = Math.sqrt(-1*t*(Math.log(alpha)-Math.log(k)-2*Math.log(t))/2);
            double gt = gt(t);
            for(int i = 0;  i < I.size(); i++) {
